...
literal: TK_LI_DECIMAL { $$ = asd_new($1->valor, TYPE_DECIMAL);
				free($1->valor);
				free($1);}
 
       | TK_LI_INTEIRO { $$ = asd_new($1->valor, TYPE_INTEIRO);
				$$->temp = new_temp();
                                $$->code = iloc_new_list();
                                append_instr($$->code, new_instr("loadI", $1->valor, NULL, $$->temp));
				free($1->valor);
				free($1);} ;
...
cs_declaracao_variavel: TK_VAR TK_ID TK_ATRIB tipo
			...
			{ $$ = asd_new("com", $4->type);
				...
				if ($4->type == TYPE_DECIMAL && $6->type == TYPE_INTEIRO){
			       ...
			       asd_add_child($$, $6);
				Symbol *sym = lookup_symbol($2->valor);
        			$$->code = iloc_new_list();
				$$->temp = new_temp();
				append_instr($$->code, new_instr("loadI", $6->label, NULL, $$->temp));
				append_instr($$->code, new_instr("storeAI", $$->temp, "rfp", sym->offset_str));
			       free($2->valor);
			       free($2);
			       asd_free($4); 
			};
...
cs_comando_atribuicao: TK_ID TK_ATRIB expressao 
			{Symbol *sym = lookup_symbol($1->valor);
			 ...
			 $$ = asd_new(":=", sym->type);
				...
				$$->temp = $3->temp;
				if (is_global(sym)) {
					append_instr($$->code, new_instr("storeAI", $$->temp, "rbss", sym->offset_str));
				} else {
					append_instr($$->code, new_instr("storeAI", $$->temp, "rfp", sym->offset_str));
				}
			 } ;
...
condicional: TK_SE '(' expressao ')' cria_escopo cs_bloco_de_comandos remove_escopo senao_opcional
		{ $$ = asd_new("se", $3->type);
			...
			append_instr($$->code, new_instr("cbr", $$->temp, L_true, L_false));
			append_instr($$->code, new_label_instr(L_true));
			if ($6 != NULL) {
				$$->code = concat($$->code, $6->code);
			}	

			if ($8 != NULL) {
				append_instr($$->code, new_instr("jumpI", NULL, NULL, L_end));
				append_instr($$->code, new_label_instr(L_false));
				$$->code = concat($$->code, $8->code);
				append_instr($$->code, new_label_instr(L_end));
			} else {
...
repeticao: TK_ENQUANTO '(' expressao ')' cria_escopo cs_bloco_de_comandos remove_escopo
		{ $$ = asd_new("enquanto", $3->type);
			...
			$$->code = iloc_new_list();
			append_instr($$->code, new_label_instr(L_begin));
			$$->code = concat($$->code, $3->code);
			$$->temp = $3->temp;
		
			append_instr($$->code, new_instr("cbr", $$->temp, L_body, L_end));
			append_instr($$->code, new_label_instr(L_body));
			$$->code = concat($$->code, $6->code);

			append_instr($$->code, new_instr("jumpI", NULL, NULL, L_begin));
			append_instr($$->code, new_label_instr(L_end));
		} ;
...
expr_or:
  expr_or '|' expr_and { if ($1->type != $3->type) {
			 ...
			 $$ = asd_new("|", TYPE_INTEIRO);
				...
				$$->temp = new_temp();
				append_instr($$->code, new_instr("or", $1->temp, $3->temp, $$->temp)); } 
...
expr_and: 
  expr_and '&' expr_eq { if ($1->type != $3->type) {
			 ...
			 $$ = asd_new("&", TYPE_INTEIRO);
				...
				$$->code = concat($1->code, $3->code);
				$$->temp = new_temp();
				append_instr($$->code, new_instr("and", $1->temp, $3->temp, $$->temp)); } 
...
expr_eq:
  expr_eq TK_OC_EQ expr_rel { if ($1->type != $3->type) {
				...
				$$ = asd_new("==", TYPE_INTEIRO);
					...
					$$->code = concat($1->code, $3->code);
					$$->temp = new_temp();
					append_instr($$->code, new_instr("cmp_EQ", $1->temp, $3->temp, $$->temp)); } 
...
| expr_eq TK_OC_NE expr_rel { if ($1->type != $3->type) {
				...
				$$ = asd_new("!=", TYPE_INTEIRO);
					...
					$$->code = concat($1->code, $3->code);
					$$->temp = new_temp();
					append_instr($$->code, new_instr("cmp_NE", $1->temp, $3->temp, $$->temp)); } 
...
expr_rel:
  expr_rel '<' expr_add { if ($1->type != $3->type) {
			  ...
			  $$ = asd_new("<", TYPE_INTEIRO);
				...
				$$->code = concat($1->code, $3->code);
				$$->temp = new_temp();
				append_instr($$->code, new_instr("cmp_LT", $1->temp, $3->temp, $$->temp)); } 
...
| expr_rel '>' expr_add { if ($1->type != $3->type) {
			  ...
			  $$ = asd_new(">", TYPE_INTEIRO);
				...
				$$->code = concat($1->code, $3->code);
				$$->temp = new_temp();
				append_instr($$->code, new_instr("cmp_GT", $1->temp, $3->temp, $$->temp)); } 
...
| expr_rel TK_OC_LE expr_add { if ($1->type != $3->type) {
				...
				$$ = asd_new("<=", TYPE_INTEIRO);
					...
					$$->code = concat($1->code, $3->code);
					$$->temp = new_temp();
					append_instr($$->code, new_instr("cmp_LE", $1->temp, $3->temp, $$->temp)); } 
...
| expr_rel TK_OC_GE expr_add { if ($1->type !=  $3->type) {
				...
				$$ = asd_new(">=", TYPE_INTEIRO);
					...
					$$->code = concat($1->code, $3->code);
					$$->temp = new_temp();
					append_instr($$->code, new_instr("cmp_GE", $1->temp, $3->temp, $$->temp)); } 
...
expr_add: 
  expr_add '+' expr_mul { if ($1->type != $3->type) {
			  ...
			  $$ = asd_new("+", $1->type);
				...
				$$->code = concat($1->code, $3->code);
				$$->temp = new_temp();
				append_instr($$->code, new_instr("add", $1->temp, $3->temp, $$->temp)); } 
...
| expr_add '-' expr_mul { if ($1->type != $3->type) {
			  ...
			  $$ = asd_new("-", $1->type);
				...
				$$->code = concat($1->code, $3->code);
				$$->temp = new_temp();
				append_instr($$->code, new_instr("sub", $1->temp, $3->temp, $$->temp)); } 
...
expr_mul:
  expr_mul '*' expr_unary { if ($1->type != $3->type) {
				...
				$$->code = concat($1->code, $3->code);
				$$->temp = new_temp();
				append_instr($$->code, new_instr("mult", $1->temp, $3->temp, $$->temp)); } 
...
| expr_mul '/' expr_unary { if ($1->type != $3->type) {
				...
				$$->code = concat($1->code, $3->code);
				$$->temp = new_temp();
				append_instr($$->code, new_instr("div", $1->temp, $3->temp, $$->temp)); } 
...
| expr_mul '%' expr_unary { if ($1->type != $3->type) {
			    ...
			    $$ = asd_new("%", $1->type);
				asd_add_child($$, $1);
				asd_add_child($$, $3); } // NÃO GERA CÓDIGO ILOC 
| expr_unary { $$ = $1; } ;

expr_unary: 
  '+' expr_unary { $$ = asd_new("+", $2->type);
			asd_add_child($$, $2);
			$$->code = $2->code;
			$$->temp = $2->temp; }
...
| '-' expr_unary { $$ = asd_new("-", $2->type);
			...
			$$->temp = new_temp();
			char *tzero = new_temp();
			append_instr($$->code, new_instr("loadI", "0", NULL, tzero));
			append_instr($$->code, new_instr("sub", tzero, $2->temp, $$->temp)); }

| '!' expr_unary { $$ = asd_new("!", $2->type);
			asd_add_child($$, $2);
			$$->code = $2->code;
			$$->temp = new_temp();
			append_instr($$->code, new_instr("cmp_EQ", $2->temp, "0", $$->temp)); }
...
expr_primary: TK_ID {   Symbol *sym = lookup_symbol($1->valor);
			...
			$$ = asd_new($1->valor, sym->type);
				...
				$$->temp = new_temp();
				if (is_global(sym))
					append_instr($$->code, new_instr("loadAI", "rbss", sym->offset_str, $$->temp));
        			else
					append_instr($$->code, new_instr("loadAI", "rfp", sym->offset_str, $$->temp));
				free($1->valor);
				free($1); }
